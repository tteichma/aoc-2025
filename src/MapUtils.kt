...
sealed class Direction(val nextCoordinate: (IntCoordinate) -> IntCoordinate) {
    ...
    companion object {
        ...
        private val oppositeDirs by lazy {
            mapOf(
                LR to RL, RL to LR, UD to DU, DU to UD
            )
        }
        private val perpendicularDirsLists by lazy {
            mapOf(
                LR to listOf(UD, DU), RL to listOf(UD, DU), UD to listOf(LR, RL), DU to listOf(LR, RL)
            )
        }
...
open class DataMap<T>(val data: List<List<T>>) {
    ...
    fun copyDataWithModification(coordinate: IntCoordinate, newValue: T) = data.withIndex().map { (iRow, row) ->
            row.withIndex()
    ...
    protected fun IntCoordinate.isWithinBoundaries() = (this.first in rowIndices && this.second in colIndices)

    protected fun IntCoordinate.getFourNeighbours(predicate: (IntCoordinate) -> Boolean = { true }) =
        Direction.entries.map { this@getFourNeighbours + it }.filter { it.isWithinBoundaries() }
            .filter { predicate(it) }

    protected fun IntCoordinate.getEightNeighbours(predicate: (IntCoordinate) -> Boolean = { true }) =
        listOf(-1, 0, 1).flatMap { xOffset ->
                listOf(-1, 0, 1).mapNotNull { yOffset ->
                        if (xOffset == 0 && yOffset == 0) null else this + IntCoordinate(xOffset, yOffset)
                    }
            }.filter { it.isWithinBoundaries() }.filter { predicate(it) }
    ...
    fun floodMap(start: IntCoordinate, predicate: (T) -> Boolean): Set<IntCoordinate> {
        ...
        while (toExplore.isNotEmpty()) {
            ...
            toExplore.addAll(currentCoordinate.getFourNeighbours { this[it] == expectedValue }.filterNot { it in seen })
        }
